        ...
        for prod in df_forecast['Product'].unique():
            ...
            for n in nodes:
                results_list.append({
                    ...
                    'Location': n,
                    'Period': month,
                    'Agg_Demand': agg_demand[n], # FIX: Use generic name here
...
if s_file and d_file and lt_file:
    ...
    # FUTURE NETWORK AGGREGATION
    network_stats = aggregate_network_stats(df_forecast=df_d, df_stats=stats, df_lt=df_lt)
    network_stats.rename(columns={'Agg_Demand': 'Agg_Future_Demand'}, inplace=True) # Explicit rename
    ...
    # CALCULATION
    results['SS_Raw'] = z * np.sqrt((results['LT_Mean'] / 30) * (results['Agg_Std_Hist']**2) + (results['LT_Std']**2) * (results['Agg_Future_Demand'] / 30)**2)
    ...
    if apply_cap:
        l_lim, u_lim = results['Agg_Future_Demand'] * (cap_range[0]/100), results['Agg_Future_Demand'] * (cap_range[1]/100)
    ...
    # --- HISTORICAL ACCURACY DATA PREP ---
    hist_agg = aggregate_network_stats(df_forecast=df_s, df_stats=stats, df_lt=df_lt)
    hist_agg.rename(columns={'Agg_Demand': 'Agg_Hist_Demand'}, inplace=True) # FIX: Now matches the function output
    ...
    # TABS
    # ---------------------------------------------------------
    tabs = st.tabs(["📈 Corridor", "🕸️ Network", "📋 Plan", "⚖️ Efficiency", "📉 Accuracy", "🧮 Trace"])

    with tabs[4]: # Forecast Accuracy Tab
        ...
        if not hdf.empty:
            ...
            st.dataframe(hdf[['Period','Consumption','Forecast_Hist','Agg_Hist_Demand','Deviation','Abs_Error','APE_%','Accuracy_%']], use_container_width=True)
        else:
            st.warning("No historical data found for this selection.")

    # (Other tabs remain the same as your MEIO (5).py version)
    with tabs[0]: # tab1
        plot_df = results[(results['Product'] == h_sku) & (results['Location'] == h_loc)].sort_values('Period')
        fig = go.Figure([
    ...
    with tabs[5]: # Calculation Trace (Tab 6)
        row = results[(results['Product'] == h_sku) & (results['Location'] == h_loc)].iloc[0]
        ...
        st.info(f"Aggregated Network Demand (D): {row['Agg_Future_Demand']}")
        st.success(f"Final Safety Stock: {row['Safety_Stock']} ({row['Adjustment_Status']})")
