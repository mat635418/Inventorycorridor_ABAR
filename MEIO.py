...
if s_file and d_file and lt_file:
    ...
    # -------------------------------
    # TAB 2: Network Topology (MODIFIED FOR CENTERING)
    # -------------------------------
    with tab2:
        sku_nt = st.selectbox("Product for Network View", all_products, key="nt_sku")
        per_nt = st.selectbox("Period", all_periods, key="nt_period")
        
        # --- NEW CSS INJECTION TO CENTER TOPOLOGY ---
        st.markdown("""
            <style>
                /* Target the Pyvis container and center it */
                .vis-network {
                    margin-left: auto !important;
                    margin-right: auto !important;
                }
                /* Center the iframe generated by Streamlit components */
        ...
        # Ensure width is set to 100% to allow the margin:auto logic to work within the container
...
""")
            ...
            # Add base pre-rule SS and implemented SS for reference
            ...
            compare_df = pd.concat([pd.DataFrame([base_row, impl_row]), scen_df.rename(columns={'Simulated_SS': 'Simulated_SS'})], ignore_index=True, sort=False)
            # Display comparison table (formatted)
            ...
            # Sensitivity curve: SS vs Service Level for selected scenario LT assumptions (use first scenario's LT)
            ...
            # Delta metrics for scenario 1 vs implemented/base
    ...
    # -------------------------------
    # TAB 7: By Material (ensure Part B SS Attribution present)
    # -------------------------------
    with tab7:
