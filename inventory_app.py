...
def aggregate_network_stats(df_forecast, df_stats, df_lt):
    """
    AMENDED: Calculates monthly network demand.
    Propagates FUTURE Forecast per month and historical variance from 
    ...
    for month in months:
        # Filter forecast for this specific month
        df_month = df_forecast[df_forecast['Future_Forecast_Month'] == month]
        
        for prod in df_month['Product'].unique():
            # Get historical variance (static across months in this model)
            p_stats = df_stats[df_stats['Product'] == prod].set_index('Location').to_dict('index')
            
            # Get future forecast for this specific month and product
            ...
            # Initialize with local values for this month
            ...
            # Network propagation logic (Tiers)
            for _ in range(10):
                changed = False
                for parent in nodes:
...
if s_file and d_file and lt_file:
    ...
    # 1. Calc Local Historical Stats (For Variance)
    ...
    # MERGE CHANGE: Join on Product, Location AND Month
    ...
    # SS Formula: Now dynamic based on monthly network demand
    ...
    with tab3:
        ...
        with col3: f_month = st.multiselect("Filter Month", sorted(results['Future_Forecast_Month'].unique()))
        ...
        st.dataframe(filtered_df[[
            ...
            'Agg_Future_Demand': 'Network Demand (Monthly)'
